    ...
    case 0:
	  if (HasAttribute(tag)) {
	  	switch (AttributeType(tag)) {
	  	  default:
	  	  case IntegerAttributeType: attribute = (double) Attribute(tag); break;
		  case FloatAttributeType  : attribute = (double) FloatAttribute(tag); break;
          case DoubleAttributeType : attribute = DoubleAttribute(tag); break;
		  case LongAttributeType   : attribute = (double) LongAttribute(tag); break;
	  	}
	    histo.Add(attribute);
	  }
	  break;
