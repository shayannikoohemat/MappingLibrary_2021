...
void BNF_Three_Doubles(const char *line, double *number1, double *number2,
                       double *number3)
{
  char *colon;

  colon = strchr(line, ':');
  sscanf(colon+1, "%lf%lf%lf", number1, number2, number3);
}
...
void BNF_Write_Three_Doubles(FILE *fd, const char *keyword, int indent,
...
/*
--------------------------------------------------------------------------------
                  Derive a directory name from a full file name
--------------------------------------------------------------------------------
*/

char *DeriveDirectoryFromFile(const char *file)
{
  char *slash, *directory;

  if (!file) return(NULL);
  directory = (char *) malloc((strlen(file) + 1) * sizeof(char));
