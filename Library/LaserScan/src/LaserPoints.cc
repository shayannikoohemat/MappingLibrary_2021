...
void LaserPoints::ReduceData(double min_dist, int knn, bool update_weights)
{
  ...
  // Check that all points have weights if they are to be updated
  if (update_weights) {
  	for (point=begin(); point!=end(); point++)
      if (!point->HasAttribute(WeightTag)) point->FloatAttribute(WeightTag) = 1.0;
  }
  ...
  if (size() == 2) {
    ...
    if ((point->vect() - nb_point->vect()).Length2D() <= min_dist)
      nb_point->UnSelect();
    if (update_weights)
      point->FloatAttribute(WeightTag) += nb_point->FloatAttribute(WeightTag);
  }
  ...
  // Regular case for three or more points
    ...
    if (knn) {
      ...
      if (knn > 0) knn_max = knn > size() ? size() : knn; // 3D
      else knn_max = -knn > size() ? size() : -knn; // 2D
      par.NeighbourhoodStorageModel() = 2; // knn
      par.NumberOfNeighbours() = knn_max;
      if (knn < 0) par.DistanceMetricDimension() = 2;
        ...
        for (node=list.begin(), i=0; node!=list.end(); node++, i++) {
          ...
          if (point->IsSelected()) {
            ...
            for (nb_node=nbhood.begin(); nb_node!=nbhood.end(); nb_node++) {
              ...
              if (nb_point == point) continue; // Skip the point itself if it's in its neighbourhood
              if (update_weights && nb_point->IsSelected())
                point->FloatAttribute(WeightTag) += nb_point->FloatAttribute(WeightTag);
              nb_point->UnSelect();
            }
