                 ...
                 public LaserDataFiles
{
  protected:
    ...
    /// Strip axis (sometimes used for sorting data)
    Line2D *strip_axis;
...
/* Functions specific to strips of laser data */
    ...
    /// Return the strip axis
    const Line2D *StripAxis() const
      {return strip_axis;}
    
    /// Set the strip axis
    void SetStripAxis(const Line2D &new_axis)
    ...
    */
    Line2D DetermineStripAxis(LaserPoint &start_point, LaserPoint &end_point);

    /// Determine strip axis
    /** Just fit a line to all points. For use in case of reordered data.
        @param verbose Output progress per part if true
        @return 2D line through start and end point
    */
    Line2D DetermineStripAxis(bool verbose=false);
