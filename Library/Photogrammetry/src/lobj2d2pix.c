...
/****************************************************************
* 
*  Routine Name: lobj2d2pix - This function transforms planimetric object coordinates to image coordinates given the grid parameters defining the relation between the two coordinate systems.
* 
*       Purpose: This function transforms a set of planimetric object coordinates to image 
coordinates.
...
****************************************************************/
/* -library_def */
void Object_2D_To_Image(Grid *grid, ObjPt2D *objpt2d, ImgPt *imgpt)
/* -library_def_end */
...
/* -library_code */
{
  ...
  imgpt->num = objpt2d->num;

  imgpt->r   = (grid->y - objpt2d->y) / grid->pixelsize - 0.5;
  imgpt->c   = (objpt2d->x - grid->x) / grid->pixelsize - 0.5;

  sqsize = grid->pixelsize * grid->pixelsize;

  imgpt->v_r   = objpt2d->v_y / sqsize;
  imgpt->v_c   = objpt2d->v_x / sqsize;
  imgpt->cv_rc = objpt2d->cv_xy / sqsize;
}
/* -library_code_end */
