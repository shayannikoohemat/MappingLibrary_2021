...
/*
--------------------------------------------------------------------------------
               Return the distance to another bounding box
--------------------------------------------------------------------------------
*/
double DataBounds3D::DistanceXY(const DataBounds3D &bounds) const
{
  if (OverlapXY(*this, bounds)) return 0.0;
  
  if (bounds.Maximum().X() < Minimum().X()) { // Left of this
    if (bounds.Minimum().Y() > Maximum().Y()) // Upper left
      return Vector2D(bounds.Maximum().X() - Minimum().X(),
	                  bounds.Minimum().Y() - Maximum().Y()).Length();
    else if (bounds.Maximum().Y() < Minimum().Y()) // Lower left
      return Vector2D(bounds.Maximum().X() - Minimum().X(),
	                  bounds.Maximum().Y() - Minimum().Y()).Length();
	else // Overlap in Y coordinate range
	  return Minimum().X() - bounds.Maximum().X();
  }
  else if (bounds.Minimum().X() > Maximum().X()) { // Right of this
    if (bounds.Minimum().Y() > Maximum().Y()) // Upper right
      return Vector2D(bounds.Minimum().X() - Maximum().X(),
	                  bounds.Minimum().Y() - Maximum().Y()).Length();
    else if (bounds.Maximum().Y() < Minimum().Y()) // Lower right
      return Vector2D(bounds.Minimum().X() - Maximum().X(),
	                  bounds.Maximum().Y() - Minimum().Y()).Length();
	else // Overlap in Y coordinate range
	  return bounds.Minimum().X() - Maximum().X();
  }
  else { // Overlap in X coordinate range
    if (bounds.Minimum().Y() > Maximum().Y()) // Above this
      return bounds.Minimum().Y() - Maximum().Y();
    else if (bounds.Maximum().Y() < Minimum().Y())
      return (Minimum().Y() - bounds.Maximum().Y());
    else {
      printf("DataBounds3D::DistanceXY should not get here\n");
      printf("this X range %.2f - %.2f  Y range %.2f - %.2f\n",
	         Minimum().X(), Maximum().X(), Minimum().Y(), Maximum().Y());
      printf("bounds X range %.2f - %.2f  Y range %.2f - %.2f\n",
