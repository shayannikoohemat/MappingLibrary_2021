...
/* Definition of REAL moved to include file triangle.h                       */
/* George Vosselman                                                          */
...
/* Forward declaration */
void poolrestart(struct memorypool *);
...
void enqueuebadtri(struct triedge *instri, REAL angle, point insapex,
                   point insorg, point insdest)
{
...
struct splaynode *splay(struct splaynode *splaytree, point searchpoint,
...
struct splaynode *splayinsert(struct splaynode *splayroot, 
                              struct triedge *newkey, point searchpoint)
{
...
struct splaynode *circletopinsert(struct splaynode *splayroot,
                                  struct triedge *newkey, point pa, point pb, 
                                  point pc, REAL topy)
{
  REAL ccwabc;
...
struct splaynode *frontlocate(struct splaynode *splayroot, 
                              struct triedge *bottommost, point searchpoint, 
...
int reconstruct(int *trianglelist, REAL *triangleattriblist,
                REAL *trianglearealist, int elements,
                int corners, int attribs, int *segmentlist, 
                int *segmentmarkerlist, int numberofsegments)

#else /* not TRILIBRARY */

long reconstruct(char *elefilename, char *areafilename, char *polyfilename, 
                 FILE *polyfile)
...
void segmentintersection(struct triedge *splittri, struct edge *splitshelle,
                         point endpoint2)
{
  point endpoint1;
  point torg, tdest;
...
void readholes(FILE *polyfile, char *polyfilename, REAL **hlist, int *holes,
...
void writepoly(char *polyfilename, REAL *holelist, int holes, REAL *regionlist,
...
void writevoronoi(char *vnodefilename, char *vedgefilename, int argc,
                  char **argv)
