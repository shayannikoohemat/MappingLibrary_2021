...
std::vector<LaserPoints> PartitionLpByTag(LaserPoints const& lp, LaserPointTag int_tag,
        char* output, bool write_output) {
...
void LaserPoints_info (LaserPoints &laserpoints, int lp_size){
    ...
    printf("First SegmentNumber: %d , Last SegmentNumber: %d \n", segment_numbers.front (), segment_numbers.back ());
    printf("Number of segments: %d \n", segment_numbers.size ());
    LaserPoints lp_large_segments;
    vector <LaserPoints> lp_vec;
    char * root= nullptr;
    lp_vec = PartitionLpByTag (laserpoints, SegmentNumberTag, root);
    int count=0;
    for (auto &s : lp_vec) {
        if (s.size () > lp_size) {
            count++;
            lp_large_segments.AddPoints (s);
        }
    }
    printf("#segments bigger than %d: %d \n", lp_size, count);
    //lp_large_segments.Write("E:/publication_data/temp/lp_out.laser", false);
...
LaserPoints LabelPoints_To_Ver_Horizon (LaserPoints &laserpoints, double angle_threshold){
    ...
    vector<LaserPoints> segments;
    char* root = nullptr;
    segments = PartitionLpByTag (laserpoints, SegmentNumberTag, root);
...
/// with wall, floor, ceiling labels and relabel them
/// find the result of changedetection in the original pointclouds and relabel them to walls, floor and ceiling
LaserPoints relabel_changed_points (LaserPoints &detected_changes, LaserPoints & original_labeled_points,
...
/*NOTE: lp_master should be in a separate directory of the rest of the files, otherwise it would be duplicated.
 * Lp_master cann't be empty.*/
/*void merge_lp_segmented (LaserPoints &lp_master, char *input_directory, char *output_file){

    std::vector<std::pair<string, string>> files_v;
    files_v = RecursiveDirFileNameAndPath (input_directory);

    //auto it = std::begin(files_v);  /// this is for the case that masterfile is inside the directory
    ...
    printf ("Reading directory files... \n");
    /// start with the remaining of the files to resegment and add to the previous one
    //++it;  /// this is for the case that masterfile is inside the directory
    //for(auto end=std::end(files_v); it!=end; ++it) {
    for (auto it = std::begin (files_v); it != std::end(files_v); it++){
        string file_path;
        file_path = it->first; // fielpath
        LaserPoints lp_tmp;
        vector<LaserPoints> lp_segments_v;
        lp_tmp.Read (file_path.c_str ());
        lp_segments_v = PartitionLpByTag (lp_tmp, SegmentNumberTag, input_directory);
...
LaserPoints Renumber_points_byTag (LaserPoints &laserPoints, LaserPointTag laserPointTag, int start_number){
    ...
    char * root = nullptr;
    lp_Tag_vec = PartitionLpByTag (laserPoints, laserPointTag, root);
...
LaserPoints selectpts(float minH, float maxH, LaserPoints inpts){
    //local variables
...
// slice points on Z by a given number of slices/ or percentiles
vector <LaserPoints> slicePoints_Z_axis (LaserPoints &laserPoints, LaserPoints &sliced_laserPoints,
                                            int number_of_percentiles, bool verbose){

    vector<LaserPoints> percentiles_vec;

    //laserPoints.SortOnCoordinates();
    //get minimum and maximum Z
    DataBoundsLaser        selbounds;
    if (laserPoints.size() > 4) {
        selbounds = laserPoints.DeriveDataBounds(0);
        LaserPoint minpt = selbounds.Minimum();
        LaserPoint maxpt = selbounds.Maximum();
        double zrange = selbounds.ZRange();
        double minZ = minpt.GetZ();
        double maxZ = maxpt.GetZ();
        float percZ = zrange/float(number_of_percentiles);        //height of the percentile
        if(verbose) printf ("slice height: %.2f\n", percZ);
        int component_num=0;
        for (int i=0; i < number_of_percentiles; i++){
            // get percentiles and start (minpercZ) and end (maxpercZ) by height of slice
            double minpercZ = minZ + percZ * i;
            double maxpercZ = minpercZ + percZ;
            ///printf ("MinSliceZ: %.2f    MaxSlizeZ: %.2f\n", minpercZ, maxpercZ);

            // extract points for the slice
            DataBoundsLaser newbounds;
            LaserPoints percentile_pts;

            newbounds.SetMinimumZ(minpercZ);
            newbounds.SetMaximumZ(maxpercZ);
            if(i==number_of_percentiles-1)
                newbounds.SetMaximumZ(maxZ); // if it is the last slice set the max height to the maxZbound
            /// slice points with the given points
            laserPoints.Select(percentile_pts,newbounds);
            // set component tag for this percentile
            percentile_pts.SetAttribute(ComponentNumberTag, i+1);

            percentiles_vec.push_back(percentile_pts);

            // make the output file
            sliced_laserPoints.AddPoints(percentile_pts);
        }
        return percentiles_vec;
    } else
        {
        printf ("Error: input laser doesn't have enough points! \n");
        return percentiles_vec;
    }

}

// slice points on Z by a specific given height
// slices are stored in ComponentNumberTag
vector <LaserPoints> slicePoints_by_height (LaserPoints &laserPoints, LaserPoints &sliced_laserPoints,
                                         double percentile_height, bool verbose){
    ...
    if (laserPoints.size() > 4) {
        ...
        double minZ = minpt.GetZ();
        double maxZ = maxpt.GetZ();
        int number_of_percentiles = ceil(zrange/ percentile_height);        //number of slices/percentile
        if(verbose) printf ("number of percentiles: %d\n", number_of_percentiles);
        int component_num=0;
        for (int i=0; i < number_of_percentiles; i++){
            // get the min and max of a percentile
            double minpercZ = minZ + percentile_height * i;
            double maxpercZ = minpercZ + percentile_height;
            ///printf ("MinSliceZ: %.2f    MaxSlizeZ: %.2f\n", minpercZ, maxpercZ);

            // extract points for the slice
            //LaserPoints percentile_pts = selpts(minpercZ, maxpercZ, laserPoints);
            DataBoundsLaser newbounds;
            LaserPoints percentile_pts;
...
/*
 *  make a polyline by connecting all the points in the given laserfile
 *  the laser points should already be sorted by an attribute which is used for connecting the points, for example
 *  if the Z-value is used it should be sorted by Z. Or if time is used then already sorted by time.
 *  points can be skipped by the $point_to_skip parameter.
 *  points_to_skip shouldnẗ be a zero
 * */
///
/// \param lp
/// \param points_to_skip
/// \param polyline_topo     output edges
/// \param vertices         output vertices
void Points2PolyLine (LaserPoints &lp, int points_to_skip, LineTopologies &polyline_topo, ObjectPoints &vertices){
    ...
    if (vertices.empty()){
        ...
        pointnumber = (vertices.back ()).Number () + 1;
    }
    for (auto &p : lp){
        if (points_to_skip == 0) points_to_skip=1; // to avoid division by zero
    ...
    polyline_topo.push_back (linesegments);
}
