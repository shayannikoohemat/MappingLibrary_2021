...
void Mergesurfaces (const LaserPoints &segmented_lp, double max_dist_between_planes, double max_angle_between_normals,
                    ...
                    bool calculate_middle_plane, bool calculate_weighted_plane, bool verbose=false);
...
std::vector<LaserPoints> PartitionLpByTag(LaserPoints const& lp, LaserPointTag int_tag,
        char* output, bool write_output=false);
...
// slice point clouds on the z-axis for a given number of slices/ or percentiles
// the output is sliced_laserPoints where the ComponentNumberTag has the slice number AND it returns a vec of slices
vector <LaserPoints> slicePoints_Z_axis (LaserPoints &laserPoints, LaserPoints &sliced_laserPoints,
                                         int number_of_percentiles, bool verbose=false);
...
vector <LaserPoints> slicePoints_by_height (LaserPoints &laserPoints, LaserPoints &sliced_laserPoints,
                                         double percentile_height, bool verbose=false);

// crop part of points by min and max z values
 ...
 *  points can be skipped by the $point_to_skip!=0 parameter.
 * */
