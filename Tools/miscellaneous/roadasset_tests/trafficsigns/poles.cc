...
/*          for (it=subunitptr->begin(); it!=subunitptr->end(); it++){
          ...
          for (seg_id = seg_nums.begin(); seg_id!=seg_nums.end(); seg_id++){
              ...
              sel_laser_points = lp.SelectTagValue(SegmentNumberTag, *seg_id);
              ...
              if (sel_laser_points.size()>4){
                  ...
                  if (percpts.size()>4){
                      ...
                      for (int i=0; i<=numpartstotal; i++){
                          ...
                          //check if max number of points per part is exceeded
...
*/
                      ...
                      if (rectangles.size()>1){
                         ...
                         if  (countpart >= numpart){                           
                             ...
                             pnl = GetListByTag(SegmentNumberTag,*seg_id, lp);
                             lp.Label(pnl, 1, lstag);
