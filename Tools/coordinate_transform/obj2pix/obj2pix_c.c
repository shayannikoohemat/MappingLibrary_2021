...
extern void Object_To_Image(Grid *, ObjPt *, ImgPt *);
...
int obj2pix_c(char *i_file, char *o_file, char *g_file)
{
  ImgPts   *imgpts;
  Grid     *grid;
  ObjPts   *objpts;
  ...
  if (!grid || !objpts) exit(0);

  imgpts = (ImgPts *) malloc(sizeof(ImgPts));
  imgpts->num_pts = objpts->num_pts;
  ...
  for (i=0, imgpt=imgpts->pts, objpt=objpts->pts;
       i<imgpts->num_pts;
       i++, imgpt++, objpt++)
    Object_To_Image(grid, objpt, imgpt);
