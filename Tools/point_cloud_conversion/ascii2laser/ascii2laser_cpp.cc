...
void ascii2laser_cpp(char *ascii_file, char *ascii_file_filter,
					 ...
					 int column_scanlinenumber, bool big_strip)
{
  LaserUnit              strip;
  ...
  while ((filename = get_full_filename(directory, filter, &icon)) != NULL) {
    ...
    do {
      if (fgets(line, 1024, ascii)) {
        ...
        if (column_scanlinenumber) // Scanline number
          point.SetAttribute(ScanLineNumberTag,(int) value[column_scanlinenumber]);
      ...
      if (pointset.size() == max_subunit_size) {
        ...
        if (column_p && !column_n && pointset.size() == max_subunit_size) {
