...
bool read_color_label_ascii(const string &, Point_set &, int);
void save_point_set (Point_set &, string);
...
int planar_ransac(const string &ascii_filename, Efficient_ransac::Parameters ransac_parameters,
                  Point_set &point_set, int nb_neighbors){
    ...
    /// populate PointVectorMap with the point_set and empty normals
    /// NOTE: this for loop can be integrated into read_color_label_ascii() and return both points and point_set
...
//    Point_set::iterator ps_it = point_set.begin();
//    for (ps_it = point_set.begin(); ps_it != point_set.end(); ps_it++){
//        Kernel::Point_3 point3(point_set.point(*ps_it));
...
//    /// order of the points is the same
//    Point_set::iterator pset_it = point_set.begin();
//    for (auto &p : points){
//        point_set.normal(*pset_it) = Vector(p.second.x(), p.second.y(), p.second.z());
//        pset_it++;
//    }

    // write point with normals for double check
    string out_ascii_normal="/mnt/DataPartition/threed_modeling/out/points_normal_label.txt";
    ...
    ransac.add_shape_factory<CGAL_Plane>();
     ...
     while (it != shapes.end()) {
       ...
       while (index_it != (*it)->indices_of_assigned_points().end()) {
         ...
         int inx = pointset_it - point_set.begin();
         Point p = point_set.point(inx);
         //Point &p = *(point_set.begin() + (*index_it));
