...
void City::ChangeBackGroundColour()
{
...
bool City::SetBackGroundImage(BackGroundType clicked_item,
                                          CityWindow *window)
{
...
bool City::GenerateHeightTexture(CityWindow *window)
{
...
bool City::GenerateShadedHeightTexture(CityWindow *window)
{
  Image *shaded_image;
...
void City::LoadBackGroundImage()
{
  ...
  filename = QFileDialog::getOpenFileName(this, "Select ortho image",
                                          QString::null, "Images (*.xv)");
  if (filename.isEmpty()) return;

  // Read the image
  background_image = new Image(filename.toAscii(), &success);
  ...
  // Read the image grid
  grid = new ImageGrid(filename.toAscii(), &success);
  ...
  SetBackGroundImage(NoBackGroundImage, CityWindowPtr()); // To remove old image
  if (texture_images[OrthoImage] != NULL)
