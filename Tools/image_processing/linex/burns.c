...
ImageLines llinexBurns_arg(int argcount, char **argvalues)
{
 int opt,k;
  char *pgmin;
  char *linesout;
  ...
  if(verbose)
    printf("Convert Burns lines data to ImageLines %s\n"); 
   ImageLines image_lines=RegList2ImageLines();
//Free some memory
...
bool llinexBurns(char* imagename, ImageLines &iml, int bucket_width=8, int min_num_pixels=25, double min_magnitude=5.0, double vote=0.5, int gradient_mask=0)//all other values are hardcoded
{
int argcount=12;
char** argval;
...
printf("\n");
*/
iml=llinexBurns_arg(argcount, argval);
