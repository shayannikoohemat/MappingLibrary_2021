...
void morphological_indoor (char* laserFile, double resolution, char *method){
   ...
   // auto *root = (char*) "E:/publication_data/Navvis/Kadaster/door_detection/out/";
    ...
    auto *root = (char*) "E:/publication_data/modeling/data/Haaksbergen/door_detection/" ;
   // auto *root = (char*) "../output/door_detection/"; //server path
    ...
    laserFile = (char*) "E:/publication_data/modeling/data/Haaksbergen/door_detection/walls_merged.laser";
...
/***********************************************************************************************************************/
    ...
    traj_laserFile = (char*) "E:/publication_data/modeling/data/Haaksbergen/door_detection/2ndfloor_traj.laser";
   ...
   // average_traj_height = (traj_db.Minimum().GetZ() + traj_db.Maximum().GetZ()) /2;
    ...
    for(auto &p : traj_laserpoints){
        LaserPoint new_p;
        /// for door detection: p.z() in the original trajectory should be shifted to the middle of the door height.
        /// 80cm height difference between the traj and the top of the door is good.
        double new_z;
        new_z = p.GetZ() +  0.76; // 1.60;
        new_p = p;
        new_p.Z() = new_z;
    ...
    if (method == "door_top_detection"){
       ...
       // traj_laserFile = (char*) "E:/publication_data/Navvis/Kadaster/door_detection/level2_kadaster_traj_Zoffset.laser";
       traj_laserFile = (char*) "E:/publication_data/modeling/data/Haaksbergen/door_detection/traj_Zoffset.laser";
        ...
        /// *** important NOTE: the input laserpoints should n't have irregular floor height or something below the floor,
        /// since the bbox would change. 80cm height difference between the traj and the top of the door is good.
        top_doors_all = vox.door_detection(vox_centers, traj_laserpoints, root, 2, 5 * vox_l, 5 * vox_l, 19 * vox_l, vec_ijk); //7, 7, 23, vox_l=0.12 // 5, 5, 17, l=10cm // 5, 5, 19, l=10, d=10, #top=13
    ...
    /******************************************************************************************************************/
    /// find edge_change voxels   ** incomplete code. **
    ...
    /******************************************************************************************************************/
    /* instead of this code, a combination of erosion + dilation is used. "empty_space" code is discourage to be used. */
    /*
