...
void PrintUsage()
{
  printf("Removal of segment number from all points belonging to segments\n");
  printf("smaller than the specified size\n");
  printf("Usage: unlabelsurfaces [-i <file name> OR -f <file filter>] : input files\n");
  printf("        [-minsize <minimum segment size (default: 100)]>\n");
...
int main(int argc, char *argv[])
{
  ...
  void unlabelsurfaces(char *, char *, char *, char *, bool, bool, int);
  ...
  unlabelsurfaces(args->String("-f"), args->String("-i"), args->String("-app"),
                  args->String("-odir"), args->Contains("-meta"),
                  args->Contains("-overwrite"),
				  args->Integer("-minsize", 100)); 
