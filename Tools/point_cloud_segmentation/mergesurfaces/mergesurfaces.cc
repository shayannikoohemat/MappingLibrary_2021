...
--------------------------------------------------------------------------------
 ...
 The points are written to file. Optionally, a meta data file is
 generated.
...
void mergesurfaces(char *long_filter, char *infile,
    ...
    for (unitptr=block.begin(); unitptr!=block.end(); unitptr++) {
      for (subunitptr=unitptr->begin(); subunitptr!=unitptr->end();
        ...
        if (overwrite || !BNF_FileExists(newsubunit.MetaDataFile()) ||
          ...
          timer_start(&start);
          subunitptr->MergeSurfaces(parameters);
          timer_end(start, (char *)"merging");

          // Write the points and meta data
          subunitptr->Write();
          if (output_meta_data) subunitptr->WriteMetaData();

          // Erase the points and edges
