...
|
  ...
  Merging of planar segments to smooth surfaces
...
void PrintUsage()
{
  printf("mergesurfaces merges nearly coplanar surfaces.\n");
  printf("Usage: mergesurfaces [-i <file name> OR -f <file filter>] : input files\n");
  ...
  printf("        -par <parameter file (default: mergesurfaces.par)>\n");
  printf("\nSegmentation parameter file can be constructed with PCM.\n");
}
...
int main(int argc, char *argv[])
{
  ...
  void mergesurfaces(char *, char *, char *, char *, bool, bool,
  ...
  if (args->Contains("-par")) {
    if (!par.Read(args->String("-par", "mergesurfaces.par"))) {
      printf("Error reading segmentation parameters from %s\n",
	         args->String("-par", "mergesurfaces.par"));
	  return EXIT_SUCCESS;
    }
  }
  else {
  	printf("Error: no segmentation parameter file specified with -par\n");
  	return EXIT_SUCCESS;
  }
  
  // Merge the planar segments
  mergesurfaces(args->String("-f"), args->String("-i"), args->String("-app"),
