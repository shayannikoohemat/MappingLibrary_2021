...
void PrintUsage()
{
  printf("Usage: growsegments [-i <file name> OR -f <file filter>] : input files\n");
  printf("        -par <parameter file (default: growsegments.par)>\n");
...
int main(int argc, char *argv[])
{
  ...
  void growsegments(char *, char *, char *, char *, bool, bool, bool,
  ...
  if (!par.Read(args->String("-par", "growsegments.par"))) {
  	printf("Error reading parameters from %s\n",
	       args->String("-par", "growsegments.par"));
    PrintUsage();
    return EXIT_SUCCESS;
  }
  
  // Segment the data
  growsegments(args->String("-f"), args->String("-i"), args->String("-app"),
