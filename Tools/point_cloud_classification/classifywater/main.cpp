...
/*-----------------------------------------------------------
  ...
  Classification of water areas in point clouds
...
void PrintUsage()
{
  printf("Usage: classifywater [-i <file name> OR -f <file filter>] : input files\n");
  printf("        [-odir <directory name>]            : output directory\n");
  printf("        [-app <appendix>]                   : append string for output files\n");
  printf("        [-overwrite]                        : overwrite the input files\n");
...
int main(int argc, char *argv[])
{
  ...
  void classifywater(char *, char *, char *, char *, bool, bool,
  ...
  // Try to read segmentation parameters
  if (!par.Read(args->String("-par", "segmentlaser.par"))) {
    // Use defaults if not successful
    par.SeedNeighbourhoodRadius() = 100.0;
    par.GrowingRadius() = 100.0;
  }

  // Classify the water points
  classifywater(args->String("-f"), args->String("-i"), args->String("-app"),
