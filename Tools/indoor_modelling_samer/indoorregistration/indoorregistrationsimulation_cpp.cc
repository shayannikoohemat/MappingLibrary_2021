...
void indoorregistrationsimulation_cpp(char *p_file, char *o0_file, char *o1_file,
                                      ...
                                      double noise_std,
									  int num_planes,
									  char *pl_file,char *output_sysconf_file)
{
  ...
  double               angle, degree = atan(1.0) / 45.0, angle0_start,
					   ...
					   wave_length, Moving_dir1, Moving_dir2;
 ...
 if (double_wall) {
  	// All walls captured twice
    angle0_start =  135.0 * degree;//135.0
    angle1_start =   90.0* degree;//45.0
    angle2_start = -135.0 * degree;//-135
    Moving_dir1=90.0 * degree; // rotate s1 around moving direction, 0 if perpendicular to the moving direction
    Moving_dir2=0.0 * degree;// this represent s1 in our system 
  }
  else { 
    // Ceiling and floor captured twice
    angle0_start =  135.0 * degree;
  ...
  sensor_rotation_in_frame[0] = Rotation3D(0.0, 0.0, angle0_start);// Rs0=Rz(135)  
  sensor_rotation_in_frame[1] = Rotation3D(0.0, 0.0 * degree, 0.0) *
                               Rotation3D(Moving_dir1, 0.0, 0.0) *
							   Rotation3D(0.0, scanner_rotation * degree, 0.0) *
							   Rotation3D(0.0, 0.0, angle1_start);// Rs1=Ry(0).Rx(115).Ry(0).Rz(90)							   
  sensor_rotation_in_frame[2] = Rotation3D(0.0, -90.0 * degree, 0.0) *
                               Rotation3D(Moving_dir2, 0.0, 0.0) *
							   Rotation3D(0.0, -scanner_rotation * degree, 0.0) *
							   Rotation3D(0.0, 0.0, angle2_start);// Rs2=Ry(-90).Rx(0).Ry(35).Rz(-135)
...
// Update the system configuration data
  for (sensor=0; sensor<3; sensor++) {
    ...
    system[sensor].Orientation()->Orientation3DRef() =
      Orientation3D(sensor_offset_in_frame[sensor],
