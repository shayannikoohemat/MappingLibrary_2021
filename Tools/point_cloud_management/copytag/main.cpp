...
void PrintUsage()
{
  printf("Usage: copytag -i <meta file or point file> OR\n");
  printf("               -f <file filter>\n");
  printf("               -srctag <tag number to be copied\n");
  printf("               -desttag <new tag number\n\n");
...
int main(int argc, char *argv[])
{
  ...
  void copytag_cpp(char *, char *, LaserPointTag, LaserPointTag);
  ...
  if (!args->Contains("-srctag")) {
    printf("Error: -srctag is a required argument.\n");
    PrintUsage();
    exit(0);
  }
  if (!args->Contains("-desttag")) {
    printf("Error: -desttag is a required argument.\n");
    PrintUsage();
    exit(0);
  }

  // Check attribute types
  if (AttributeType((LaserPointTag) args->Integer("-srctag", 1)) !=
      AttributeType((LaserPointTag) args->Integer("-desttag", 1))) {
    printf("Warning: Attributes are not of the same type (int, float, duoble)\n");
    printf("         This may result into incorrectly copied values.\n");
  }

  // Ropy the tags
  copytag_cpp(args->String("-f"), args->String("-i"),
              (LaserPointTag) args->Integer("-srctag", 1), 
	          (LaserPointTag) args->Integer("-desttag", 1));
