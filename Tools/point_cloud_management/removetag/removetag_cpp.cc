...
/*
--------------------------------------------------------------------------------
 Removal of tags from laser point files 
...
void removetag_cpp(char *long_filter, char *infile, int tag1,
                   int tag2, int tag3, int tag4, int tag5)
{
  ...
  while ((filename = get_full_filename(directory, filter, &icon)) != NULL) {
    ...
    for (unitptr=block.begin(); unitptr!=block.end(); unitptr++) {
      ...
      for (subunitptr=unitptr->begin();
        ...
        // Read the point data, remove the tags, write and delete the point data
        if (!subunitptr->Read(subunitptr->PointFile(), false)) {
          fprintf(stderr, "Error reading laser points from file %s\n",
                  subunitptr->PointFile());
          exit(0);
        }
        subunitptr->RemoveAttribute((LaserPointTag) tag1);
        if (tag2 != -1) subunitptr->RemoveAttribute((LaserPointTag) tag2);
        if (tag3 != -1) subunitptr->RemoveAttribute((LaserPointTag) tag3);
        if (tag4 != -1) subunitptr->RemoveAttribute((LaserPointTag) tag4);
        if (tag5 != -1) subunitptr->RemoveAttribute((LaserPointTag) tag5);
