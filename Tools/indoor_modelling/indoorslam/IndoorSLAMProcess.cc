...
void IndoorSLAMProcess(char *block_file,
                       char *sensor0_file, char *sensor1_file,
					   char *sensor2_file, char *imu_metadata,
                       ...
                       int num_intervals_per_section)
{
  ...
  void LoadNewSensorData(LaserBlock &, int &, int, double, double, double, int,
                         int &, int &, LaserPoints::iterator **, IMUUnit &,
					     IMUReadings::iterator **);
  ...
  // Read IMU meta data
  if (imu_metadata) {
  	if (!imu_datasets.ReadMetaData(imu_metadata, true)) {
  	  printf("Error reading IMU meta data from file %s\n", imu_metadata);
  	  exit(0);
  	}
  }
  ...
  // If IMU data is available, also make sure the trajectory time is within
  // the limits of the IMU data
  if (imu_metadata) {
  	// Read first IMU data set
  	imu_dataset = imu_datasets.begin();
  	if (!imu_dataset->Read()) {
  	  printf("Error reading first IMU data set %s\n", imu_dataset->IMUFile());
  	  exit(0);
  	}
  	if (imu_dataset->begin()->Time() > trajectory_start_time)
  	  trajectory_start_time = imu_dataset->begin()->Time();
  	  
  	// Read the last IMU data set
  	if (imu_datasets.size() > 1) {
  	  if (!(imu_datasets.end()-1)->Read()) {
  	    printf("Error reading last IMU data set %s\n", (imu_datasets.end()-1)->IMUFile());
  	    exit(0);
  	  }
  	}
  	if (((imu_datasets.end()-1)->end()-1)->Time() > trajectory_end_time)
  	  trajectory_end_time = ((imu_datasets.end()-1)->end()-1)->Time();
    if (imu_datasets.size() > 1) (imu_datasets.end()-1)->EraseData();
  }
  ...
  if (start_part > 0) {
    ...
    if (imu_metadata) {
      // Erase data of part 0
      imu_dataset->EraseData();
      // Set iterator to the start part IMU data
      imu_dataset = imu_datasets.begin() + start_part;
      // Read data of the start part
      if (!imu_dataset->Read()) {
      	printf("Error reading IMU data from file %s\n", imu_dataset->IMUFile());
      	exit(0);
      }
    }
  }
  ...
  SetIntervalStarts(block, current_part, start_part, trajectory_start_time, 
                    ...
                    first_available_interval, last_available_interval,
                    start_point_interval, imu_datasets, &start_imu_interval);
  ...
  for (interval=1; interval<num_intervals; interval++) {
    ...
    do {
      ...
      EstimatePoseAndPlanes(interval, local_block, false,
							...
							imu_datasets,
							start_imu_interval[interval-first_available_interval+1-num_intervals_local_current],
                            start_imu_interval[interval-first_available_interval+1]);
...
//	    local_kappa_spline = fallback_kappa_spline;
    ...
    if (NeedToAdjustSection(interval, scan_line_duration, planes, 
      ...
      EstimatePoseAndPlanes(interval, block, false, 
							...
							NULL, only_2D, scan_line_duration,
							imu_datasets,
							start_imu_interval[interval-first_available_interval+1-num_intervals_per_section],
                            start_imu_interval[interval-first_available_interval+1]);
...
//		                  block[sensor].end());
  ...
  EstimatePoseAndPlanes(num_intervals-1, block, true,
						...
						scan_line_duration, imu_datasets,
						start_imu_interval[0], // Just arbitraty iterators
                        start_imu_interval[0]); // They will not be used
