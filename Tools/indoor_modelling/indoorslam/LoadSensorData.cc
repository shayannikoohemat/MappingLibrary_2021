...
void LoadNewSensorData(LaserBlock &block, int &part_number, int start_part_number,
					   ...
					   IMUReadings::iterator **start_imu_interval_ptr)
{
  void SetIntervalStarts(LaserBlock &, int, int, double, double, double, int, int &,
                         LaserPoints::iterator **, IMUUnit &,
					     IMUReadings::iterator **);

  int                   sensor, index_new_start, index_old_start, i;
  LaserUnit::iterator   old_part, new_part;
  LaserPoints::iterator point;
  IMUUnit::iterator     old_imu_part, new_imu_part;
  IMUReadings::iterator imu_reading;
  bool                  found, debug=false;
  double                switch_time, start_time;
  char                  part_name[100];
  IMUReadings::iterator *start_imu_interval=*start_imu_interval_ptr;
  ...
  // Change data for the IMU readings
  if (imu_datasets.size()) {
  	old_imu_part = imu_datasets.begin() + part_number;
  	new_imu_part = old_imu_part + 1;
  	
    // Adjust the end time of the old part and start time of the new part 
    old_imu_part->SetTimeRange(old_imu_part->StartTime(), switch_time);
	new_imu_part->SetTimeRange(switch_time, new_imu_part->EndTime());  

    // Determine the index of the first IMU reading to be kept
    index_new_start = std::distance(old_imu_part->begin(),
	                       start_imu_interval[last_available_interval - 
                     num_intervals_per_section + 1 - first_available_interval]);
	  	
  	// Remove the data that should not be saved
  	// For the first part this is data before the start of the trajectory
  	if (part_number == start_part_number)
  	  old_imu_part->SetTimeRange(trajectory_start_time, switch_time);
  	
	// For later parts this is data which was copied from earlier parts and
  	// was processed already earlier. This is data before the switch time to
  	// the old part set in the previous call to this function.
  	start_time = old_imu_part->StartTime();
  	for (imu_reading=old_imu_part->begin(), found=false;
		 imu_reading!=old_imu_part->end() && !found; imu_reading++) {
	  if (imu_reading->Time() >= start_time) found = true;
	}
	if (!found) {
	  printf("Error: start time of part not found in part IMU reading set %s\n",
		     old_imu_part->Name());
	  exit(0);
	}
	index_old_start= std::distance(old_imu_part->begin(), imu_reading);
	old_imu_part->erase(old_imu_part->begin(), imu_reading);
  	
  	// Save the data of the old part
  	if (old_imu_part->Name())
  	  sprintf(part_name, "temp_%s", old_imu_part->Name());
  	else {
	  sprintf(part_name, "temp_part%5d", part_number);
  	  for (i=9; i<13; i++)
  	    if (part_name[i] == ' ') part_name[i] = '0';
  	}
  	old_imu_part->SetName(part_name);
  	old_imu_part->DeriveIMUDataFileName("./");
  	old_imu_part->Write();
  	
    // Delete data of the old part, but keep the last num_intervals_per_section
	// intervals
  	old_imu_part->erase(old_imu_part->begin(),
	                    old_imu_part->begin() + index_new_start - index_old_start);
	  
	// Read data of the next part
	if (!new_imu_part->Read()) {
	  printf("Error reading IMU sensor data from file %s\n",
	         new_imu_part->IMUFile());
	  exit(0);
	}
	
	// Insert data to be kept from the old part 
	new_imu_part->insert(new_imu_part->begin(), old_imu_part->begin(), old_imu_part->end());
	old_imu_part->EraseData();
  }
  ...
  SetIntervalStarts(block, part_number, start_part_number, trajectory_start_time, 
                    ...
                    first_available_interval, last_available_interval,
                    start_point_interval, imu_datasets, start_imu_interval_ptr);
...
void SetIntervalStarts(LaserBlock &block, int part_number, int start_part_number,
					   ...
					   LaserPoints::iterator **start_point_interval,
					   IMUUnit &imu_datasets,
					   IMUReadings::iterator **start_imu_interval)
{
  ...
  IMUUnit::iterator     imu_dataset;
  IMUReadings::iterator start_imu_reading;
  ...
  // Do the same for IMU data, if available
  if (!imu_datasets.empty()) {
  	imu_dataset = imu_datasets.begin() + part_number;
    time = imu_dataset->EndTime();
 	if (time < part_end_time) part_end_time = time;
  }
  // Don't go beyond the trajectory end time
  ...
  // Determine the first IMU reading of every time interval to be processed
  if (imu_datasets.size()) {
  	// Allocate memory for array of IMU reading iterators
  	if (part_number == start_part_number)
  	  *start_imu_interval =
	    (IMUReadings::iterator *) malloc((num_intervals+1) *
		                                 sizeof(IMUReadings::iterator));
	else
  	  *start_imu_interval =
	    (IMUReadings::iterator *) realloc(*start_imu_interval, 
		                                  (num_intervals+1) *
		                                  sizeof(IMUReadings::iterator));
    if (!*start_imu_interval) {
      printf("Error allocating start_imu_interval vector\n");
	  exit(0);
    }
	start_imu_reading = imu_dataset->begin();
	for (interval=0, time=part_start_time; interval<=num_intervals; 
	     interval++, time+=interval_duration) {
	  while (start_imu_reading->Time() < time) start_imu_reading++;
	  (*start_imu_interval)[interval] = start_imu_reading;
	}
  }
}
