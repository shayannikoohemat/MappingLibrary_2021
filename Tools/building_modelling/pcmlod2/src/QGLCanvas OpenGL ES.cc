...
{
/* Testing for perspective projection
   glMatrixMode(GL_PROJECTION);
   glLoadIdentity();
   glFrustum(-data_range/2.0, data_range/2.0,-data_range/2, data_range/2.0, 2, 90);      // Left=-2, Right=2, Bottom=-2, Top=2, Near=5, Far=9
   PrintMatrices("after glFrustrum");
   glMatrixMode(GL_MODELVIEW);
   glLoadIdentity();
   gluLookAt(0, 0, data_range*5, 0, 0, 0, 0, 1, 0); // 0 0 6 0 0 5 0 1 0, eye, look at, up vector
   PrintMatrices("after gluLookAt");
   zoom_factor = 2.0;
   glScalef(zoom_factor, zoom_factor, zoom_factor);
   PrintMatrices("after scale");
...
*/    
  ...
  glMatrixMode(GL_MODELVIEW); // test
  glPushMatrix(); // test
  glScalef(zoom_factor, zoom_factor, zoom_factor);
  glTranslatef(translation[0], translation[1], 0.0);
  glRotatef(rotation[0], 1.0, 0.0, 0.0);
  ...
  glLoadIdentity(); // test
  if (perspective) {
    printf("pixel_size %.4f data_range %.2f zoom_factor %.2f X-off %.2f Y-off %.2f\n",
           pixel_size, data_range, zoom_factor, translation[0], translation[1]);
    glFrustum(-2,2,-2,2,5,9);      // Left=-2, Right=2, Bottom=-2, Top=2, Near=5, Far=9
/*
    glTranslatef(0.0, 0.0, 2*data_range); // Back off by data_range
    glFrustum(-pixel_size * width() / 4.0,  pixel_size * width() / 4.0,
              -pixel_size * height() / 4.0, pixel_size * height() / 4.0,
              0.5*data_range ,     10*data_range );
*/
  }
  else
  ...
  if (appearance->ShowLines()) {
    ...
    for (polygon=top->begin(); polygon!=top->end(); polygon++) {
      ...
      if (appearance->LineColourMethod() == ColourByLabel) {
        ...
        colour_index = (*polygon)->Number() - ((*polygon)->Number() / 216) * 216;
        ...
        if ((*polygon)->HasAttribute(BuildingNumberTag))
          colour_index = (*polygon)->Attribute(BuildingNumberTag) -
                         ((*polygon)->Attribute(BuildingNumberTag) / 216) * 216;
        else colour_index = 0;
        glColor3fv(random_colours[colour_index]);
      }
        ...
        if ((*polygon)->HasAttribute(BuildingPartNumberTag))
          colour_index = (*polygon)->Attribute(BuildingPartNumberTag) -
                         ((*polygon)->Attribute(BuildingPartNumberTag) / 216) * 216;
        else colour_index = 0;
        glColor3fv(random_colours[colour_index]);
      }
      ...
      if (appearance->TypeOfData() == TileBoundaryData) {
        ...
        renderText(canvas_point.x()+10, canvas_point.y()+20,
                   QString(tile_name));
      }
    }
  }
...
//Old GL      glColor3f(appearance->PointColour().red()   / 255.0,
//Old GL                appearance->PointColour().green() / 255.0,
//Old GL                appearance->PointColour().blue()  / 255.0);
      glColor4f(appearance->PointColour().red()   / 255.0,
                appearance->PointColour().green() / 255.0,
                appearance->PointColour().blue()  / 255.0, 1.0); // OpenGL ES
    glPointSize((GLfloat) appearance->PointSize());
    ...
    if (button_down[0] || button_down[1] || button_down[2])
      ...
      point_increment = 1;
//Old GL    glBegin(GL_POINTS); 
    vertices = (GLfloat *) malloc(3* laser_points->size() * sizeof(GLfloat)); // OpenGL ES
    glVertexPointer(3, GL_FLOAT, 0, vertices); // OpenGL ES
    glEnableClientState(GL_VERTEX_ARRAY); // OpenGL ES
    iv = 0; // OpenGL ES
...
//Old GL        glVertex3f((float) (laser_point->X() - offset.X()),
//Old GL                   (float) (laser_point->Y() - offset.Y()),
//Old GL                   (float) (laser_point->Z() - offset.Z()));
        vertices[iv++] = (GLfloat) (laser_point->X() - offset.X()); // OpenGL ES
        vertices[iv++] = (GLfloat) (laser_point->Y() - offset.Y()); // OpenGL ES
        vertices[iv++] = (GLfloat) (laser_point->Z() - offset.Z()); // OpenGL ES
      }
    }
//Old GL    glEnd();
    glDrawArrays(GL_POINTS, 0, laser_points->size()); // OpenGL ES
  }
    ...
    for (mesh=laser_points->TINReference().begin();
      ...
      if (!skip) {
        ...
        switch (appearance->FaceColourMethod()) {
          ...
          case ColourByHeight:
            // Average height
            ColourAttribute((laser_point->Z() + laser_point2->Z() +
                             laser_point3->Z()) / 3.0,
                            appearance->PointColourCycleLength(),
                            appearance->PointColourCyclePhase(),
                            red, green, blue);
