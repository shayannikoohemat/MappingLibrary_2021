...
{
  QGLCanvas::TranslateCanvas(x_perc, y_perc, false);
...
{
  QGLCanvas::ScaleCanvas(scale_factor, false);
  emit NewCanvasBounds(BoundsXY());
  QGLCanvas::update();
}
...
void PCMCanvas::SelectLaserData(LaserPoints &points,
                                LaserPoints &selected_points,
                                bool select_inside, bool set_IsSelectedTag) const
{
...
//Old GL  glColor3f(0.0, 1.0, 0.0);
  glColor4f(0.0, 1.0, 0.0, 1.0);  // OpenGL ES
//Old GL  glBegin(GL_LINES);
                              ...
                              Vector3D &viewdir)
{
  ...
  GLfloat    vertices[6]; // OpenGL ES

  glVertexPointer(3, GL_FLOAT, 0, vertices); // OpenGL ES

  // The main scale line
//Old GL  glVertex3f(pt1.X(), pt1.Y(), pt1.Z());
//Old GL  glVertex3f(pt2.X(), pt2.Y(), pt2.Z());
  vertices[0] = pt1.X();
  vertices[1] = pt1.Y();
  vertices[2] = pt1.Z();
  vertices[3] = pt2.X();
...
//               footpt.Y() - crossdir.Y(),
//               footpt.Z() - crossdir.Z());
    vertices[0] = footpt.X() + crossdir.X();
    vertices[1] = footpt.Y() + crossdir.Y();
    vertices[2] = footpt.Z() + crossdir.Z();
    vertices[3] = footpt.X() - crossdir.X();
    vertices[4] = footpt.Y() - crossdir.Y();
    vertices[5] = footpt.Z() - crossdir.Z();
    glDrawArrays(GL_LINES, 0, 2);
  }
}
