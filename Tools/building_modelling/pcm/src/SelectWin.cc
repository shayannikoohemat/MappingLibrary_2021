                           ...
                           QWidget *parent) : QWidget(parent)
{
  ...
  // Validator
  double_validator = new QDoubleValidator(-1.0e10, 1.0e10, 4, this);
  ...
  for (i=0; i<3; i++) {
    ...
    point_minimum_line[i]->setValidator(double_validator);
    point_minimum_line[i]->setGeometry(172, y, 70, 18);
          
    // Maximum value
    point_maximum_line[i] = new QLineEdit("", this);
    point_maximum_line[i]->setValidator(double_validator);
  ...
  for (i=0; i<3; i++) {
    ...
    segment_minimum_line[i]->setValidator(double_validator);
    segment_minimum_line[i]->setGeometry(172, y, 70, 18);
          
    // Maximum value
    segment_maximum_line[i] = new QLineEdit("", this);
    segment_maximum_line[i]->setValidator(double_validator);
  ...
  for (int i=0; i<num_attributes; i++) {
  	...
  	switch (AttributeType((LaserPointTag) attributes[i])) {
  	  ...
  	  case DoubleAttributeType:
  	  	if (minima[i] != (double) INT_MIN) {
  	  	  switch ((LaserPointTag) attributes[i]) {
  	  	  	case XCoordinateTag: laser_appearance->DisplayFilter().SetMinimumX(minima[i]); break;
  	  	  	case YCoordinateTag: laser_appearance->DisplayFilter().SetMinimumY(minima[i]); break;
  	  	  	case ZCoordinateTag: laser_appearance->DisplayFilter().SetMinimumZ(minima[i]); break;
  	  	  	default: laser_appearance->DisplayFilter().Minimum().DoubleAttribute((LaserPointTag) attributes[i]) =
  	  	             minima[i]; break;
  	  	  }
  	  	}
  	  	if (maxima[i] != (double) INT_MAX) {
  	  	  switch ((LaserPointTag) attributes[i]) {
  	  	  	case XCoordinateTag: laser_appearance->DisplayFilter().SetMaximumX(maxima[i]); break;
  	  	  	case YCoordinateTag: laser_appearance->DisplayFilter().SetMaximumY(maxima[i]); break;
  	  	  	case ZCoordinateTag: laser_appearance->DisplayFilter().SetMaximumZ(maxima[i]); break;
  	  	  	default: laser_appearance->DisplayFilter().Maximum().DoubleAttribute((LaserPointTag) attributes[i]) =
  	  	             maxima[i]; break;
  	  	  }
  	  	}
  	  	break;
