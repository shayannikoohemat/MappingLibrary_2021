...
void CityGMLHandler::endElement( const std::string& wlocalname ) 
{
	...
	// unknown node ? skip now to avoid the buffer triming pass
	...
	case NODETYPE( BridgePart ):
		MODEL_FILTER();
		if ( _currentCityObject && ( _currentCityObject->size() > 0 
			|| _currentCityObject->getChildCount() > 0 
			|| !_params.pruneEmptyObjects ) ) {	// Prune empty objects 
	...
	case NODETYPE( posList ):
		...
		if ( _currentRing ) 
			...
			//parseVecList(_buff, _currentRing->getVertices() );
		break;
