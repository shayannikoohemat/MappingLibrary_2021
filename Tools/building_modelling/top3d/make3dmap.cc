...
--------------------------------------------------------------------------------
 ...
 Modified by Sander Oude Elberink, 2006-2011
 
 adjusted to 3D top10 project. assuming ahn-2 seperated datasets (either ground
 points or veg/buildings points.)
 Top10 classes:
       terrain, road, water, building (top10 building is providing foundation),
       forrest
 Bag   buildings; on top of top10 buildings.
 buildings and forrest can be put on top of terrain (+water/roads/foundation buildings)
...

{
  ...
  LaserPoint                   nb_laser_point, temp_point;
  TINEdges                     edges, seledges, poledges;
  ObjectPoints                 map_points, dtm_tin_points, clean_map_points;
  LineTopologies               map_lines, dtm_tin_lines, wall_map_lines;
  ...
  double min_height_difference = 1.5; //1.0//pcp1.5
  double other_nbh_radius      = 5.0;
  double road_nbh_radius       = 5.0;//25 pcp10
...
// Derive the TIN edges of the laser points
 ...
 if (only_buildings){
  int                number_offset2, number_offset, size, next_pnr;
  double mean, fixed_floor_height, added_floorheight = 0;
  ObjectPoints       sel_map_points, all_tin_building_points;
  ...
  for (map_line=map_lines.begin(), count=0;
       ...
       if (added_floorheight!=0) printf("warning, floorheight added by %4.2f m", added_floorheight);
       ...
       if (sel_laser_points.size()>map_line->size()){
       if (add_groundfloor){
  ...
  //        map_tin_lines.SetAttribute(LineLabelTag, map_line->Attribute(IDNumberTag));
       ...
       if(!sel_map_points.empty()) sel_map_points.erase(sel_map_points.begin(),sel_map_points.end());
       node2 = map_line->begin();
       this_point = map_points.PointIterator(*node2);        
       new_point = *this_point;             
       fixed_floor_height = new_point.Z() + 0;
       rasterise_points = false;
        if (!rasterise_points){   
       for (node2=map_line->begin(); node2!=map_line->end(); node2++) {
             this_point = map_points.PointIterator(*node2);        
             new_point = *this_point;             
          ...
          size = sel_map_points.size();
              ...
              for (x=min_x; x<max_x; x+=int(grid_size)){
                  for (y=min_y; y<max_y; y+=int(grid_size)){
                      ...
                      } while (neighbourhood.size()<3 && nbh_radius<200);
...
//                         poly_laser_points.push_back(temp_point);
                      ...
                      temp_point = LaserPoint(x-0.5*grid_size + 0.01, y-0.5*grid_size + 0.01, loc_laser_points.Mean().Z());
                       if (temp_point.InsidePolygon(map_points,
                                         map_line->LineTopologyReference())) {
                         temp_point.Attribute(PolygonNumberTag) = map_line->Number();
                         temp_point.Attribute(LabelTag)         = map_line->Label();
              ...
              for (node2=map_line->begin(); node2!=map_line->end(); node2++) {
                  this_point = map_points.PointIterator(*node2);        
                  new_point = *this_point;
                  pos = Position3D(new_point.X(), new_point.Y(),0);
                  nearest_laser_point = sel_laser_points.NearestPoint(pos,
                                            poledges, true);
                  new_point.Z() = sel_laser_points[nearest_laser_point].Z();
                  new_point.Number() = number_offset2;
                  number_offset2++;
                  node2->Number()=new_point.Number();
                  sel_map_points.push_back(new_point);
              ...
              for (i=0;i<sel_laser_points.size();i++) {
                sel_map_points.push_back(ObjectPoint(sel_laser_points[i].vect(), number_offset2, Covariance3D()));
                number_offset2 ++;
              }
          ...
          if (sel_map_points.size()>2){ 
            ...
            map_tin_lines.SetAttribute(LineLabelTag, map_line->Label());
            map_tin_lines.insert(map_tin_lines.end(), wall_map_lines.begin(), wall_map_lines.end());

//            map_tin_lines.AddWalls(one_map_line, number_offset);
//            map_tin_lines.insert(map_tin_lines.end(), one_map_line.begin(), one_map_line.end()); ?? necessary?
            map_tin_lines.SetAttribute(BuildingNumberTag, map_line->Number());
    //        map_tin_lines.SetAttribute(LineLabelTag, map_line->Attribute(IDNumberTag));
            map_tin_lines.SetAttribute(IDNumberTag, map_line->Attribute(IDNumberTag));
            if (!all_tin_building_points.empty()){
              map_tin_lines.ReNumber(sel_map_points, (all_tin_building_points.end()-1)->Number()+1, (all_tin_building_lines.end()-1)->Number()+1);
  ...
  if (!all_tin_building_lines.Write(map_topology_output, false))
    printf("Error writing the topology of the 3D map lines\n"); 
    dxffile = fopen ("buildings_dxflabel.dxf","w");
    all_tin_building_points.WriteDXFMesh(dxffile, all_tin_building_lines, 7, false, true, true, true);
    fclose(dxffile);
    fd = VRML2_Open("buildings.vrml");
    all_tin_building_points.VRML2_Write(fd, all_tin_building_lines, 1, 1, 0, 0); 
    VRML2_Close(fd);
    return;           
}
if (only_foundation){
  ...
  for (map_line=map_lines.begin(), count=0;
       ...
       sel_laser_points.ErasePoints();                                                                              
       for(nb_node = map_line->begin(); nb_node!=map_line->end(); nb_node++){
              new_point = *(map_points.PointIterator(*nb_node));         
            nearest_laser_point = laser_points.NearestPoint(new_point.Position3DRef(),
                                                    edges, true);
            nbh_radius = 3;
            do{
       ...
       for (node2=map_line->begin(); node2!=map_line->end(); node2++) {
         ...
         //    new_point.Z() = mean;
          ...
          if (sel_map_points.size()>2){ 
            ...
            number_offset = sel_map_points.HighestPointNumber().Number()+1;
            sel_map_points.DuplicateWithFixedZ(fixed_floor_height, number_offset);
            map_tin_lines.SetAttribute(LineLabelTag, map_line->Label()+4);
            wall_map_lines.erase(wall_map_lines.begin(), wall_map_lines.end());
            wall_map_lines.AddWalls(one_map_line, number_offset);
            wall_map_lines.SetAttribute(LineLabelTag, 5);
            map_tin_lines.SetAttribute(LineLabelTag, map_line->Label());
...
//            map_tin_lines.SetAttribute(LineLabelTag, map_line->Label()+4);
  ...
  if (!all_tin_building_lines.Write(map_topology_output, false))
    printf("Error writing the topology of the 3D map lines\n"); 
    dxffile = fopen ("buildings_foundation.dxf","w");
    all_tin_building_points.WriteDXFMesh(dxffile, all_tin_building_lines, 7, false, true, true, true);
    fclose(dxffile);
    fd = VRML2_Open("buildings_foundation.vrml");
    all_tin_building_points.VRML2_Write(fd, all_tin_building_lines, 1, 1, 0, 0); 
    VRML2_Close(fd);
    return;           
}
...
//            map_tin_lines.insert(map_tin_lines.end(), one_map_line.begin(), one_map_line.end()); ?? necessary?
  ...
  if (!all_tin_building_lines.Write(map_topology_output, false))
    printf("Error writing the topology of the 3D map lines\n"); 
    dxffile = fopen ("forest_dxflabel.dxf","w");
    all_tin_building_points.WriteDXFMesh(dxffile, all_tin_building_lines, 7, false, true, true, true);
    fclose(dxffile);
    fd = VRML2_Open("forest.vrml");
    all_tin_building_points.VRML2_Write(fd, all_tin_building_lines, 1, 1, 0, 0); 
    VRML2_Close(fd);
    return;           
}
...
//      if (map_line->TOP10MajorClass() == TOP10_Road) {// just for testing road reconstruction                                                      
 ...
 //                                                   edges, true);
    ...
    for (sel_map_line=sel_map_lines.begin(); sel_map_line!=sel_map_lines.end();
      ...
      poly_laser_points.AddTaggedPoints(laser_points,
                                         (*sel_map_line)->Number(),
                                         PolygonNumberTag);
//      area = (*sel_map_line)->CalculateArea(map_points); 
      if (10 * poly_laser_points.size()<(*sel_map_line)->CalculateArea(map_points)) emptymap = true;
      ...
      // For water just get all points inside the polygon
      if (top10class == TOP10_Water && !emptymap) {
      ...
      // For buildings & emptymaps, get a minimum set of points (outside polygon, on dtm level)
      if (emptymap || top10class == TOP10_Building ){
      nearest_laser_point = laser_points.NearestPoint(map_point->Position3DRef(),
                                                    edges, true);
        iter = 0; done = false;
      ...
      if ((top10class == TOP10_Meadow || top10class == TOP10_Road) && !emptymap) {
           printf("poly ");
        poly_laser_points.ErasePoints();
  //      poly_laser_points = laser_points.SelectTagValue(PolygonNumberTag,  (*sel_map_line)->Number());
        poly_laser_points.AddTaggedPoints(laser_points,
                                         (*sel_map_line)->Number(),
                                         PolygonNumberTag);
        poledges.erase(poledges.begin(), poledges.end());
      ...
      // For water and buildings, just take the lowest or highest point
      if (top10class == TOP10_Water || top10class == TOP10_Building){
 ...
 /*           seledges.Derive(sel_laser_points.TINReference());                          
            nearest_laser_point = sel_laser_points.NearestPoint(map_point->Position3DRef(),
                                                    seledges, true);
            printf("4 (%d)", nearest_laser_point);                                                              
            seledges.erase(seledges.begin(), seledges.end());
            printf("5 ");                                                              
            nb_laser_point = sel_laser_points[nearest_laser_point];
            printf("6 ");                                                              
  //          heights.push_back(nb_laser_point.Z()); 
  ...
  */        if (top10class == TOP10_Water)
          ...
          if (top10class == TOP10_Building)//else // TOP10_Building
            heights.push_back(sel_laser_points.DataBounds().Maximum().Z());
    ...
    if (debug) printf("start while loop with %d heights\n", heights.size());
      ...
      for (index1=0, min_diff=1e10; index1<heights.size()-1; index1++) {
        if (use_map_line[index1] == index1 && heights[index1] > -100.0) { //?? change to 100.0
          for (index2=index1+1; index2<heights.size(); index2++) {
            if (use_map_line[index2] == index2 &&
                heights[index2] > -100.0) { //?? change to 100.0
      ...
      if (debug) printf("Height difference 1 = %5.2f\n", min_diff);
    ...
    //                   && min_diff < 2*min_height_difference)
        ...
        // Merge heights depending on polygon classification
          if (sel_map_lines[close_index1]->TOP10MajorClass() == TOP10_Road) {
            ...
            // Two road segments, just take average
            heights[close_index1] = (heights[close_index1] + 
                                     heights[close_index2]) / 2.0;
            nb_laser_point.X() = map_point->X();
            nb_laser_point.Y() = map_point->Y();
...
//            nb_laser_point.SetPointNumber(map_point->Number());
            ...
            if (debug) printf("Merging road and other segment\n");
            for (index1=0; index1<=heights.size(); index1++)
              if (use_map_line[index1] == close_index2)
                use_map_line[index1] = close_index1;
                nb_laser_point.X() = map_point->X();
...
//            nb_laser_point.SetPointNumber(map_point->Number());
 ...
 /*                     meadow_laser_points.ErasePoints();
                      meadow_laser_points.AddTaggedPoints(laser_points,
                                         sel_map_lines[close_index1]->Number(),
                                         PolygonNumberTag); 
                      meadow_laser_points.AddTaggedPoints(laser_points,
                                         sel_map_lines[close_index2]->Number(),
                                         PolygonNumberTag); 
        //              printf("size of laser points: %d\n", meadow_laser_points.size());                   
             //         meadow_laser_points.DeriveTIN();
       /*               edges.Derive(meadow_laser_points.TINReference());
                      meadow_laser_points.FilterOnMorphology(kernel, 20, 0.1, 0, edges);
                      printf("filtered points ");                   
                      meadow_laser_points.RemoveFilteredPoints();
                      printf("removed filtered points ");                   
                      meadow_laser_points.DeriveTIN();
                      printf("derived new tin ");                   
         *//*           dtm_tin_points.erase(dtm_tin_points.begin(), dtm_tin_points.end());
                      dtm_tin_lines.erase(dtm_tin_lines.begin(), dtm_tin_lines.end());
                      dtm_tin_points = meadow_laser_points.ConstructObjectPoints();
                      meadow_laser_points.ErasePoints();
                      ...
                      for (map_line=dtm_tin_lines.begin(); map_line!=dtm_tin_lines.end(); map_line++) {
                          if (nb_laser_point.InsidePolygon(dtm_tin_points, map_line->LineTopologyReference())){
                               nb_node=map_line->begin();
                               p1 = Position3D(*(dtm_tin_points.PointIterator(*nb_node))); nb_node++;
                               p2 = Position3D(*(dtm_tin_points.PointIterator(*nb_node))); nb_node++;
                               p3 = Position3D(*(dtm_tin_points.PointIterator(*nb_node)));
                               plane = Plane(p1, p2, p3);
                               heights[close_index1] = plane.Z_At(nb_laser_point.X(), nb_laser_point.Y(), &success);
            //                     map_point->Z() = plane.Z_At(nb_laser_point.X(), nb_laser_point.Y(), &success);
                          }
                      }
                      nb_laser_point.Z()=plane.Z_At(nb_laser_point.X(), nb_laser_point.Y(), &success);
                      nb_laser_point.SetPointNumber(map_point->Number());
              */        nb_laser_point.X()=map_point->X();
                      nb_laser_point.Y()=map_point->Y();
                      nb_laser_point.Z() = (heights[close_index1] + heights[close_index2]) / 2.0; 
...
*/
  ...
  for (map_line=map_lines.begin(); map_line!=map_lines.end(); map_line++) {
    if (map_line->TOP10MajorClass() == TOP10_Road) {
      ...
      for (; node!=map_line->end()-1; next_node=map_line->NextNode(node)) {
        ...
        dist2 = (map_point->vect2D() - next_point->vect2D()).Length();
        if ((previous_point->Z() - map_point->Z()) / dist1 > max_road_slope &&
            (next_point->Z() - map_point->Z()) / dist2 > max_road_slope) {
          new_map_point = *map_point;
 ...
 clean_map_points.RemoveDoublePoints(map_lines, 0.1);
  ...
  /* this can better be done at the beginning of a next process
     for (map_line=map_lines.begin(), count=0; map_line!=map_lines.end(); map_line++) {
      if (map_line->Attribute(ToBeRemovedTag)==1){
        map_lines.erase(map_line);
        map_line--;
        count++;
      }
  }
  printf("Removed %d polygons out of %d which were on the border or already processed\n", count, count+map_lines.size());
  */
}
